...
주소 뒤에 `#settings` 를 붙이면 설정이 바로 열립니다 (평소에는 오른쪽 위 톱니바퀴).

### 페이지에 비밀번호 걸기

설정 → **페이지 비밀번호** 에서 켤 수 있습니다. 켜 두면 페이지를 열 때 비밀번호를 묻고,
맞힌 브라우저는 30일 동안 다시 묻지 않습니다. 비밀번호 자체는 저장하지 않고
되돌릴 수 없는 해시(PBKDF2·20만 회)만 `config.json` 에 남습니다.

> **완전한 잠금이 아닙니다.** 이 페이지는 서버 없이 파일만 내려 주는 GitHub Pages 이고
> 저장소도 공개라, 저장소 주소(`github.com/<사용자>/<저장소>`)를 아는 사람은 비밀번호와
> 무관하게 `job-monitor/data/state.json` 등 원본을 그대로 볼 수 있습니다.
> 모르는 사람이 우연히 들여다보는 것을 막는 **가림막**으로 쓰세요.
> 데이터까지 감추려면 저장소를 비공개로 돌리고 다른 곳(예: Cloudflare Pages + Access)에
> 올려야 합니다. 참고로 설정을 **바꾸는 것**은 지금도 GitHub 토큰이 있어야만 됩니다.
