...
확인은 **누가 깨우느냐 × 주기가 지났느냐** 두 가지로 정해집니다.

| 깨우는 쪽 | 언제 | 역할 |
| --- | --- | --- |
| cron-job.org | 매일 아침 **8시**(한국) | 평소 확인 |
| GitHub 예약 | 매일 오전 **11시**(한국, `0 2 * * *` UTC) | 아침 신호가 실패했을 때의 예비 |

깨어나도 **확인 주기(`check_interval_hours`, 현재 20시간)** 가 지난 사이트만 실제로 확인합니다.
그래서 아침 8시에 확인이 되면 11시 예비 신호는 그냥 지나가고, 메일은 **하루 한 통**으로 모입니다.
20시간으로 둔 이유는, 아침 8시 신호에서는 항상 주기가 지나 있고(24시간 전 확인) 같은 날
두 번 확인되는 일은 없기 때문입니다. 아침을 놓쳐 11시에 확인했더라도 다음 날 8시에 제자리로 돌아옵니다.

더 자주 받고 싶으면 **cron-job.org 의 실행 간격**과 이 주기를 함께 줄이세요.
둘 중 하나만 줄이면 바뀌지 않습니다.
